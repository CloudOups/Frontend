...
export class PannierService {
  ...
  addToCart(theCartItem: ElementPanier) {

    // check if we already have the item in our cart
    let alreadyExistsInCart: boolean = false;
    let existingCartItem: ElementPanier | undefined = undefined;
  ...
  logCartData(totalPriceValue: number, totalQuantityValue: number) {
    ...
    for (let tempCartItem of this.cartItems) {
      ...
      console.log(`name: ${tempCartItem.name}, quantity=${tempCartItem.quantite}, unitPrice=${tempCartItem.prixUnitaire}, subTotalPrice=${subTotalPrice}`);
    }
